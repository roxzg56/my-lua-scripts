...
local function UpdateCounterText(botCount, playerCount)
    ...
    "BOT : %d     PLAYER : %d     TOTAL : %d\n\n<font color=\"#FFD700\">MOD BY @AKOIJAMOP</font>", 
    botCount, playerCount, total
)
...
local function CounterTick()
    pcall(function()
        if not _G.CheatsEnabled then return end  -- <-- Ye line add karo agar cheats off par counter na dikhe
        local uCon = slua_GameFrontendHUD and slua_GameFrontendHUD:GetPlayerController()
...
if isValid(pc) and pc.AddGameTimer and pc ~= _G._FeaturesTimerPC then
  ...
  pc:AddGameTimer(0.15, true, function() -- Timer thoda slow (0.1 se 0.15)
    pcall(function()
      ...
      -- FOV Slider Logic (Lightweight, zaroori hai)
      ...
      -- =============== FIX 1 APPLIED: HEAVY PHYSICS LOOP REMOVED ===============
      -- Pehle yahan saare enemies ke bones resize hote the (har 0.1 sec),
      -- jisse bahut lag aata tha. Isko hata diya gaya hai.
      -- Ab yahan sirf light-weight FOV aur Grass settings chal rahi hain.
      -- AUR KYA CHAHIYE? AIMBOT + ZERO RECOIL + ESP + CHAMS BILKUL THEEK CHALENGE.
      -- ==========================================================================
    end)
  end)
end
...
local function ApplyHardAimbot()
    ...
    pcall(function()
        ...
        entity.ShotGunHorizontalSpread = 0
        entity.ShotGunVerticalSpread = 0
        ...
        -- 🔽 AIMBOT LOW : Slow speed, kam range, body par aim
        if entity.AutoAimingConfig then
            for _, range in ipairs({"OuterRange", "InnerRange"}) do
                local cfg = entity.AutoAimingConfig[range]
                if cfg then
                    cfg.Speed = 1.5            -- pehle 5 tha
                    cfg.RangeRate = 1.0        -- pehle 2 tha
                    cfg.SpeedRate = 1.5
                    cfg.RangeRateSight = 1.0
                    cfg.SpeedRateSight = 2.0
                    cfg.CrouchRate = 2.0
                    cfg.ProneRate = 2.0
                    cfg.DyingRate = 0

                    cfg.adsorbMaxRange = 60    -- kam range
                    cfg.adsorbMinRange = 10
                    cfg.adsorbMinAttenuationDis = 50
                    cfg.adsorbMaxAttenuationDis = 2000
                    cfg.adsorbActiveMinRange = 10
                end
            end
        end

        -- 🔽 Bone ko "head" se "neck" ya "chest" karein (low aim)
        pcall(function()
            ...
            if isValid(aimComp) and aimComp.Bones then
                pcall(function() 
                    aimComp.Bones[0] = "neck"    -- head ki jagah neck
                    aimComp.Bones[1] = "chest" 
                    aimComp.Bones[2] = "pelvis"
                end)
                pcall(function()
                    aimComp.Bones:Set(0, "neck")
                    aimComp.Bones:Set(1, "chest")
                    aimComp.Bones:Set(2, "pelvis")
                end)
            end
        end)
